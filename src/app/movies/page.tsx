...
import { getPopularMovies, getTopRatedMovies, getNowPlayingMovies, getImageUrl, getGenreNameAr } from '@/lib/tmdb';
...
export default async function MoviesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    const totalPages = Math.min(moviesData.total_pages, 500); // TMDB limits to 500 pages

    return (
        <div className="min-h-screen">
            {/* Header */}
            <div className="bg-gradient-to-b from-red-900/30 to-transparent pt-8 pb-16">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">الأفلام</h1>
                    <p className="text-gray-400 text-lg">اكتشف أحدث وأفضل الأفلام العربية والأجنبية</p>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Link
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${sort === 'now_playing'
                                        ? 'bg-red-600 text-white'
                                        : 'bg-white/10 text-gray-300 hover:bg-white/20'
                                    }`}
                            >
                                ...
                                <div className="p-3">
                                    ...
                                    <div className="flex items-center gap-2 mt-1 text-sm text-gray-400">
                                        ...
                                        {movie.genre_ids?.[0] && (
                                            <>
                                                <span>•</span>
                                                <span>{getGenreNameAr(movie.genre_ids[0])}</span>
                                            </>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="flex justify-center gap-2 mt-12 mb-8">
                    {currentPage > 1 && (
                        <Link
                            href={`/movies?sort=${sort}&page=${currentPage - 1}`}
                    ...
                    <span className="px-6 py-3 bg-red-600 text-white rounded-lg">
                        {currentPage}
                    </span>
